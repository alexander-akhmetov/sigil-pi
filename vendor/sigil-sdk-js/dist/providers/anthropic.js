...
function anthropicMessagesFromStream(request, summary, options = {}) {
    ...
    const result = summary.finalResponse
        ...
        : {
            ...
            tools: mappedRequest.tools,
            metadata: mergeMetadata(metadataWithThinkingBudget(options.metadata, mappedRequest.thinkingBudget), streamUsageMetadata),
            tags: options.tags ? { ...options.tags } : undefined,
        };
    if (options.rawArtifacts) {
...
function mapAnthropicRequest(request) {
    ...
    return {
        ...
        maxTokens: readIntFromAny(request.max_tokens),
        temperature: readNumberFromAny(request.temperature),
        topP: readNumberFromAny(request.top_p),
        toolChoice: canonicalToolChoice(request.tool_choice),
