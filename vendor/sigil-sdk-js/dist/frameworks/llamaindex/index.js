...
export function createSigilLlamaIndexHandler(client, options = {}) {
    return new SigilLlamaIndexHandler(client, options);
}
export function attachSigilLlamaIndexCallbacks(callbackManager, client, options = {}) {
    const existing = registrations.get(callbackManager);
    if (existing !== undefined) {
        return existing;
    }
    const handler = createSigilLlamaIndexHandler(client, options);
    const llmRunIds = new Map();
    const llmFallbackRunIds = [];
    const toolRunIds = new Map();
    const toolFallbackRunIds = [];
    let sequence = 0;
    const nextRunId = (prefix) => `${prefix}:${++sequence}`;
    const onLlmStart = (event) => {
        safelyRun(async () => {
            ...
            await handler.handleChatModelStart({ name: 'llamaindex.llm' }, [messages], runId, undefined, {
                invocation_params: {
                    stream,
                },
            }, undefined, buildMetadata(event, detail), 'llamaindex.llm');
        });
    };
    ...
    const onToolCall = (event) => {
        safelyRun(async () => {
            ...
            await handler.handleToolStart({
                name: toolName,
            }, read(toolCall, 'input'), runId, undefined, undefined, buildMetadata(event, detail, { event_id: callId }), toolName);
        });
    };
    ...
    const onQueryStart = (event) => {
        safelyRun(() => runChainStart(handler, event, {
    ...
    const onQueryEnd = (event) => {
        safelyRun(() => runChainEnd(handler, event, 'llama_query'));
    };
    const onSynthesizeStart = (event) => {
        safelyRun(() => runChainStart(handler, event, {
    ...
    const onSynthesizeEnd = (event) => {
        safelyRun(() => runChainEnd(handler, event, 'llama_synthesize'));
    };
    const onRetrieveStart = (event) => {
        safelyRun(async () => {
            const detail = asRecord(event.detail);
            const eventId = resolveEventId(detail);
            if (eventId.length === 0) {
            ...
            await handler.handleRetrieverStart({ name: 'llamaindex.retrieve' }, normalizeMessageText(read(read(detail, 'query'), 'query')), runId, undefined, undefined, buildMetadata(event, detail), 'llamaindex.retrieve');
        });
    };
    const onRetrieveEnd = (event) => {
        safelyRun(async () => {
            const detail = asRecord(event.detail);
            const eventId = resolveEventId(detail);
            if (eventId.length === 0) {
...
export function withSigilLlamaIndexCallbacks(config, client, options = {}) {
    const base = { ...(config ?? {}) };
...
async function runChainStart(handler, event, config) {
    const detail = asRecord(event.detail);
    const eventId = resolveEventId(detail, config.idPath);
    if (eventId.length === 0) {
        return;
    }
    await handler.handleChainStart({ name: config.name }, undefined, `${config.runPrefix}:${eventId}`, undefined, undefined, buildMetadata(event, detail), config.runType, config.name);
}
async function runChainEnd(handler, event, runPrefix, idPath) {
    const detail = asRecord(event.detail);
...
function buildMetadata(event, detail, extras) {
    const eventId = resolveEventId(detail);
...
function resolveConversationId(detail) {
    const candidates = [
...
function resolveEventId(detail, idPath) {
...
function resolveModelName(response) {
...
function resolveStopReason(response) {
    const raw = read(response, 'raw');
...
function resolveUsage(response) {
    const raw = read(response, 'raw');
...
function resolveStreamFlag(detail) {
...
function normalizeLlamaMessages(messages) {
