...
const DISABLED: Readonly<SigilPiConfig> = {
  enabled: false,
  endpoint: "",
  auth: { mode: "none" },
  agentName: "pi",
...
export function resolveConfig(file: Record<string, unknown>): SigilPiConfig {
  const enabled = envBool("SIGIL_PI_ENABLED") ?? toBool(file.enabled) ?? false;
  if (!enabled) return disabledConfig();
  ...
  if (!endpoint) {
    console.warn("[sigil-pi] enabled but endpoint is missing — disabling");
