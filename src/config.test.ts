...
describe("resolveConfig", () => {
  ...
  it("returns disabled when enabled but no endpoint", () => {
    const cfg = resolveConfig({ enabled: true });
    expect(cfg.enabled).toBe(false);
  });

  it("returns disabled when endpoint is whitespace", () => {
    const cfg = resolveConfig({ enabled: true, endpoint: "   " });
    expect(cfg.enabled).toBe(false);
  });
  ...
  it("env vars override file values", () => {
    process.env.SIGIL_PI_ENABLED = "true";
  ...
  it("env SIGIL_PI_ENABLED=0 disables", () => {
    process.env.SIGIL_PI_ENABLED = "0";
    const cfg = resolveConfig({
      enabled: true,
      endpoint: "http://localhost:8080/api/v1/generations:export",
    });
  ...
  it("disables on bearer auth with missing token", () => {
    const cfg = resolveConfig({
      enabled: true,
      endpoint: "http://localhost:8080/api/v1/generations:export",
  ...
  it("disables on basic auth with missing fields", () => {
    const cfg = resolveConfig({
      enabled: true,
      endpoint: "http://localhost:8080/api/v1/generations:export",
  ...
  it("disables on unsupported auth mode", () => {
    const cfg = resolveConfig({
      enabled: true,
      endpoint: "http://localhost:8080/api/v1/generations:export",
      auth: { mode: "berer" },
